...
import type { LookupProposal, VesselLookupResponse } from '../api/intake';

/**
 * What an outside source found about a hull, and what to believe of it.
 *
 * <b>Everything here is marked as coming from off the premises, deliberately and
 * repeatedly.</b> A deadweight accepted from a public ship database is indistinguishable from
 * one a broker checked the moment it lands in the column — so the origin is on the card, on
 * every value, on the button that writes it, and in the change set the write creates. The
 * vessel's History tab will say "Web lookup (vesselfinder) IMO 9014561" beside the field months
 * later, which is the only way the question "where did this figure come from" has an answer at
 * all.
 *
 * <b>Two screens show this and there is one of it.</b> A review item asks about a hull an email
 * raised a question over; a vessel's own record asks about the ship somebody has open. The
 * evidence, the confidence and the wording that separates "agrees on four things" from "agrees
 * on the name it was given" have to read identically in both, or the same percentage means two
 * things depending on where it is printed. What differs between the callers is only what a
 * search costs and what there is to write to, and those arrive as props.
 *
 * <b>Applying is its own button.</b> Never folded into a save or into accepting an email: two
 * origins, two writes, two change sets.
 *
 * The confidence figure is shown with what it rests on rather than alone: a hull matched on the
 * name and nothing else scores 100%, because the one thing that could be checked agreed — and
 * the name is what was searched for, so that is the query coming back rather than evidence.
 * `corroborated` is what separates the two, and it is said in words.
 */
export default function LookupCard({
  ...
  searching: boolean;
  applying: boolean;
  /** False when there is no record to write to yet — the button is shown and disabled. */
  canApply: boolean;
  onSearch: () => void;
  onApply: (fields: string[]) => void;
  /** Shown before anything has been searched. What a search costs differs per caller. */
  emptyHint: ReactNode;
  /** Shown for a recorded non-search. Only the review queue produces these. */
  skippedHint?: ReactNode;
  /** What to do about an IMO already on another hull — the most valuable thing a search finds. */
  onFileHint?: (name?: string) => ReactNode;
  /** Why applying is not possible, when it is not. */
  blockedHint?: ReactNode;
  /**
   * The ticked fields, when the screen around this card is holding them.
   *
   * <b>Lifted so one button can answer a whole screen.</b> The review drawer's footer fires the
   * web's figures and then the email's, each keeping its own change set; this card keeps its own
   * button for taking the web's figures without answering the item at all.
   */
  chosen?: string[];
  ...
  const search = (
    <Button size="small" icon={<SearchOutlined />} loading={searching} onClick={onSearch}>
  ...
  return (
    <Card size="small" style={{ marginBottom: 16 }} title={header} extra={search}>
      ...
      {lookup.status === 'SKIPPED' && (
        <Typography.Text type="secondary" style={{ fontSize: 12 }}>
          {skippedHint ?? 'Nothing was asked, because a search could only have agreed.'}
        </Typography.Text>
      )}
        ...
        <>
          {lookup.onFileVesselId && (
            <Alert
              ...
              description={
                onFileHint?.(lookup.onFileVesselName) ??
                'A vessel here already carries this IMO. She is not a new hull — she has been renamed.'
              }
            />
          )}
          ...
          {proposals.length === 0 ? (
            ...
            <>
              ...
              <div style={{ marginTop: 8 }}>
                <Typography.Text type="secondary" style={{ fontSize: 11 }}>
                  Written as its own change, named for its source, so the vessel&rsquo;s History
                  tab shows which figures came off the web and from where.
                </Typography.Text>
              </div>
            </>
          )}
